...
def _shortcut(input_feature, residual, conv_name_base=None, bn_name_base=None):
    ...
    if stride_width > 1 or stride_height > 1 or not equal_channels:
        ...
        elif conv_name_base is None:
            print('shortcut name wrong')
...
def _block_name_base(stage, block, name = 'counting'):
    ...
    def f(input_features):
        ...
        if is_first_block_of_first_layer:
            ...
            x = residual_unit(filters=filters, kernel_size=(3, 3),
                              ...
                              bn_name=bn_name_base + '2a')(input_features)
        ...
        x = residual_unit(filters=filters, kernel_size=(3, 3),
                          ...
                          bn_name=bn_name_base + '2b')(x)

        return _shortcut(input_features, x, conv_name_base = conv_name_base + 'c', bn_name_base=bn_name_base + 'b')
    ...
    def f(input_feature):
        ...
        if is_first_block_of_first_layer:
            ...
            x = residual_unit(filters=filters, kernel_size=(1, 1),
                              ...
                              dilation_rate=dilation_rate,
                              conv_name=conv_name_base + '2a',
                              bn_name=bn_name_base + '2a')(input_feature)
        ...
        x = residual_unit(filters=filters * 4, kernel_size=(1, 1),
                          ...
                          bn_name=bn_name_base + '2c')(x)

        return _shortcut(input_feature, x, conv_name_base = conv_name_base + 's', bn_name_base = bn_name_base + 'b')
...
def ResNet(input_shape=None, classes=10, block='bottleneck', residual_unit='v2',
           ...
           final_pooling=None, top='classification', name = None):
    ...
    x = _conv_bn_relu(filters=initial_filters, kernel_size=initial_kernel_size,
                      strides=initial_strides, conv_name = appstr(name, 'c1'), bn_name = appstr(name, 'b1'))(img_input)
    ...
    # Last activation
    x = _bn_relu(block, bn_name=appstr(name, 'br_last'))
...
def ResNet18(input_shape=(224,224,16), classes = 7, name = 'counting'):
    """ResNet with 18 layers and v2 residual units
    """
    model = ResNet(input_shape, classes, 'basic', repetitions=[2, 2, 2, 2], name = appstr(name, '_ResNet'))
    input = Input(input_shape)
    output = model(input)
    d1 = Dense(128, activation='relu', kernel_initializer="he_normal", name=appstr(name, 'd1'))(output)
    d2 = Dense(28, activation='relu', kernel_initializer="he_normal", name=appstr(name, 'd2'))(d1)
    d3 = Dense(7, activation='relu', kernel_initializer="he_normal", name=appstr(name, 'd3'))(d2)
    d4 = Dense(1, activation='linear', name=appstr(name, 'd4'))(d3)
    res18 = Model(input,d4, name = appstr(name,'r1'))
    return res18


def ResNet34(input_shape=(224,224,16), classes = 7, name = 'counting'):
    """ResNet with 34 layers and v2 residual units
    """
    model =  ResNet(input_shape, classes, 'basic', repetitions=[3, 4, 6, 3], name = appstr(name, '_ResNet'))
    input = Input(input_shape)
    output = model(input)
    d1 = Dense(128, activation='relu', kernel_initializer="he_normal", name=appstr(name, 'd1'))(output)
    d2 = Dense(28, activation='relu', kernel_initializer="he_normal", name=appstr(name, 'd2'))(d1)
    ...
    res34 = Model(input, d4, name = appstr(name, 'r1'))
    return res34


def ResNet50(input_shape=(224,224,16), classes = 7, name = 'counting'):
    """ResNet with 50 layers and v2 residual units
    """
    model = ResNet(input_shape, classes, 'bottleneck', repetitions=[3, 4, 6, 3], name = appstr(name, '_ResNet'))
    input = Input(input_shape)
    output = model(input)
    d1 = Dense(1000, activation='relu', kernel_initializer="he_normal", name = appstr(name, 'd1'))(output)
    d2 = Dense(500, activation='relu', kernel_initializer="he_normal", name = appstr(name, 'd2'))(d1)
    d3 = Dense(128, activation='relu', kernel_initializer="he_normal", name = appstr(name, 'd3'))(d2)
    d4 = Dense(28, activation='relu', kernel_initializer='he_normal', name = appstr(name, 'd4'))(d3)
    d5 = Dense(7, activation='relu', kernel_initializer='he_normal', name = appstr(name, 'd5'))(d4)
    d6 = Dense(1, activation='linear', name = appstr(name, 'd6'))(d5)

    res50 = Model(input, d6, name = appstr(name, 'r1'))
    return res50

def ResNet101(input_shape=(224,224,16), classes = 7, name = 'counting'):
    """ResNet with 101 layers and v2 residual units
    """

    model = ResNet(input_shape, classes, 'bottleneck', repetitions=[3, 4, 23, 3], name = appstr(name, '_ResNet'))
    input = Input(input_shape)
    output = model(input)
    d1 = Dense(1000, activation='relu', kernel_initializer="he_normal", name=appstr(name, 'd1'))(output)
    d2 = Dense(500, activation='relu', kernel_initializer="he_normal", name=appstr(name, 'd2'))(d1)
    ...
    res101 = Model(input, d6)
    return res101

def ResNet152(input_shape=(224,224,16), classes = 7):
