...
from utils.math import linspace_2d
from utils.activations import channel_softmax_1d
from utils.activations import channel_softmax_2d
...
def conv2d(x, filters, kernel_size, strides=(1, 1), padding='same', name=None, activation = None, kernel_initializer = 'glorot_uniform'):
    """Conv2D possibly wrapped by a TimeDistributed layer."""
    f = Conv2D(filters, kernel_size, strides=strides, padding=padding,
            use_bias=False, name=name, activation=activation,kernel_initializer = kernel_initializer)
    # he_initializer =


    #print('K.ndim(x)\t',K.ndim(x))  == 5
    return TimeDistributed(f, name=name)(x) if K.ndim(x) == 5 else f(x)


def sepconv2d(x, filters, kernel_size, strides=(1, 1), padding='same', activation = None,
        name=None):
...
def avgpool2d(x, pool_size = (2,2), strides = None, padding = 'valid', name = None):
    f = AveragePooling2D(pool_size=pool_size, strides = strides, padding = padding, name = name)

    return TimeDistributed(f, name=name)(x) if K.ndim(x) == 5 else f(x)

def zeropadding2d(x, padding = (1,1), name = None):
    f = ZeroPadding2D(padding = padding, name = name)
    return TimeDistributed(f, name=name)(x) if K.ndim(x) == 5 else f(x)


def upsampling2d(x, kernel_size=(2, 2), data_format='channels_last', interpolation='nearest',name=None):
    """UpSampling2D possibly wrapped by a TimeDistributed layer."""
    # f = UpSampling2D(kernel_size, interpolation=interpolation,name=name)
...
def conv_bn(x, filters, size, strides=(1, 1), padding='same', name=None, kernel_initializer = 'glorot_uniform'):
    if name is not None:
    ...
    x = conv(x, filters, size, strides, padding, conv_name, kernel_initializer = kernel_initializer)
    x = BatchNormalization(axis=-1, scale=True, name=bn_name)(x)
    return x
...
def conv_bn_act(x, filters, size, strides=(1, 1), padding='same', name=None, kernel_initializer='glorot_uniform'):
    if name is not None:
        conv_name = name + '_conv'
        bn_name = name + '_bn'
        ac_name = name + 'relu'
    ...
    x = conv(x, filters, size, strides, padding, name = conv_name, kernel_initializer=kernel_initializer)
    x = BatchNormalization(axis=-1, scale=False, name=bn_name)(x)
    x = Activation('relu', name=ac_name)(x)
    return x

# def conv_bn_act_hm(x, filters, size, strides=(1, 1), padding='same', name=None, kernel_initializer='glorot_uniform'):
