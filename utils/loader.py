...
#from exp.pennaction.penn_newannot import penn_newannot
...
class BatchLoader(Sequence):
    ...
    def __init__(self, dataset, x_dictkeys, y_dictkeys, mode,
        ...
        self.num_stages = num_predictions//4
    ...
    def __len__(self):
        ...
        for d in range(self.num_datasets):
            dataset_len.append(
                    int(np.floor(self.datasets[d].get_length(self.mode) /
                        float(self.batch_sizes[d]))))

        return max(dataset_len)

    """get batch_size info at one time, the total times is calculated by _len_, 1 batch info is gathered in get_data function"""
